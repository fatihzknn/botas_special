...
export class ConnectDbService {
  ...
  insertEmployee(data: Employee) {
    const body = {
      ...
      Operation: 'insert',
      Encrypted: 1951,
      Data: `Insert into "postgres".public.botas_cik(sicil_no,ad_soyad,birim,alt_birim,unvan,beden,ayak_no,kan_grubu,cinsiyet,ilk_yardim,aktif_pasif) values(${data.sicil_no}, '${data.ad_soyad}', '${data.birim}','${data.alt_birim}','${data.unvan}',${data.beden},${data.ayak_no},'${data.kan_grubu}','${data.cinsiyet}','${data.ilk_yardim}','Aktif')`,
    };
  ...
  deleteEmployee(sicil_no: any) {
      const body = {
        ...
        Operation: 'delete',
        Encrypted: 1951,
        Data: `delete from \"postgres\".public.botas_cik where sicil_no=${sicil_no}`
      };
      return this.http.post(baseUrl + 'Applications/DataOps', body)
    ...
    updateEmployee(data:Employee){
      ...
      const body = {
        ...
        Encrypted: 1951,
        Data:
          `Update \"postgres\".public.botas_cik Set sicil_no=${data.sicil_no},ad_soyad='${data.ad_soyad}',birim='${data.birim}',alt_birim='${data.alt_birim}',unvan='${data.unvan}',beden='${data.beden}',ayak_no='${data.ayak_no}',kan_grubu='${data.kan_grubu}',cinsiyet='${data.cinsiyet}',ilk_yardim='${data.ilk_yardim}',aktif_pasif='${data.aktif_pasif}'
    ...
    aktifToPasif(sicil_no: any){
      const body = {
        ...
        Encrypted: 1951,
        Data:
        `Update \"postgres\".public.botas_cik Set aktif_pasif='Pasif'
    ...
    return this.http.post(baseUrl + 'Applications/DataOps', body);
      ...
      pasiftoAktif(sicil_no: any){
        const body = {
          ...
          Encrypted: 1951,
          Data:
          `Update \"postgres\".public.botas_cik Set aktif_pasif='Aktif'
          WHERE sicil_no = ${sicil_no}`
      };
      ...
      return this.http.post(baseUrl + 'Applications/DataOps', body);
        ...
        getAktifEmployees() {
          const body = {
            ...
            Operation: 'read',
            Encrypted: 1951,
            Data: `select * from \"postgres\".public.botas_cik where aktif_pasif='Aktif'`
          };
          return this.http.post(baseUrl + 'Applications/DataOps', body).pipe(
            map((response: any) => {
              return response.message;
        ...
        getSpecialClothes() {
          const body = {
            ...
            Operation: 'read',
            Encrypted: 1951,
            Data: `select sicil_no,ad_soyad,birim,unvan,beden,ayak_no,kan_grubu,cinsiyet,ilk_yardim from \"postgres\".public.botas_cik where sicil_no=${this.getUserInformation2}`,
          };
          return this.http.post(baseUrl + 'Applications/DataOps', body).pipe(
            map((response: any) => {
              return response.message;
